...
import { X, DollarSign, Tag, ArrowUpCircle, ArrowDownCircle, Loader2 } from "lucide-react";
import toast from "react-hot-toast";

export default function AddTransactionModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  const [description, setDescription] = useState("");
  const [displayAmount, setDisplayAmount] = useState("0,00"); // O que o usuário vê
  const [numericAmount, setNumericAmount] = useState(0);      // O que o Firebase salva
  const [type, setType] = useState<'income' | 'expense'>('income');
  const [loading, setLoading] = useState(false);

  // Função para formatar o valor em tempo real (Máscara)
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold transition-all cursor-pointer ${
                type === 'income' ? 'bg-white text-green-600 shadow-sm scale-[1.02]' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold transition-all cursor-pointer ${
                type === 'expense' ? 'bg-white text-red-600 shadow-sm scale-[1.02]' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-slate-400 group-focus-within:text-blue-500 transition-colors">R$</span>
              <input
                type="text"
                inputMode="numeric"
                value={displayAmount}
          ...
          {/* Descrição */}
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 ml-1">O que é?</label>
            <div className="relative group">
